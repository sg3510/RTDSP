...
float snr_inter_min = 0;                  /* Stores current SNR global min */
float snr_inter_max = 0;                  /* Stores current SNR global max */
float snr_inter_range = 0;                /* Store current SNR range */
int   VAD_on = 1;                         /* Allows the code to turn VAD off */
...
{      
      ...
      {                           
            ...
            #endif
            #if (optim6 == 0)
            //if optim6 not enable set all the coefficients to one such that they do not have any effect
            lpfcoef[k] = 1;
            #endif
...
//|||||||||||||||||||||||||||||||||||||Added Code Here|||||||||||||||||||||||||||||||||   
      ...
      { 
            ...
            #if optim8 == 1
              if( set == 1)
              {//if N[k-1]/X[k-1] > residue for C[k-1] then take min of adjancent frames
      ...
      if (snr_count >= f_count_loop/3){
        ...
        //If power opf nosie under 1 we can assumed noise has not been correctly detected so far and VAD must be turned off
        if (N_Power <= 1)
        {
             VAD_on = 0;
             Threshold_coef = .05;
